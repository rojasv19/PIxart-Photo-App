...
import { User, ServerStorageStats, StudioBrandingConfig } from '../types';
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <nav className={`hidden md:flex items-center gap-1 p-1 rounded-xl border backdrop-blur-md ${
            ...
            {currentUser?.role === 'admin' || currentUser?.role === 'photographer' ? (
              ...
              /* When Client is logged in: Static Title instead of button */
              <div
                id="nav-client-title"
              ...
              /* When Logged Out: Keep Acceso con PIN in the menu for quick code entrance */
              <>
                <button
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {/* Server Storage Indicator for Admin */}
            {(currentUser?.role === 'admin' || currentUser?.role === 'photographer') && (
              <div 
                id="server-storage-badge"
                onClick={() => onNavigate('admin-storage')}
                title={`Almacenamiento: ${formatBytes(storageStats.usedBytes)} / ${formatBytes(storageStats.totalCapacityBytes)}`}
                className={`hidden lg:flex items-center gap-2.5 px-3 py-1.5 rounded-xl border cursor-pointer backdrop-blur-md transition-colors ${
                  isHomeView
                    ? 'bg-black/35 border-white/15 text-slate-200 hover:bg-white/15'
              ...
              >
                ...
                <div className="text-left">
                  <div className="flex items-center justify-between gap-3 text-[11px]">
                    <span className={isHomeView ? 'text-slate-300' : isDark ? 'text-slate-400' : 'text-slate-500'}>Espacio:</span>
                    <span className="font-semibold font-mono-code">{formatBytes(storageStats.usedBytes)}</span>
                  </div>
                  <div className={`w-20 h-1.5 rounded-full overflow-hidden mt-0.5 ${isDark || isHomeView ? 'bg-slate-800/80' : 'bg-slate-300'}`}>
                    <div 
                      className={`h-full rounded-full ${colorTheme.twBg}`} 
