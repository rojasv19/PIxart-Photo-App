...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'galleries' | 'clients' | 'storage' | 'permissions' | 'branding'>(initialTab);
  ...
  return (
    <div className={`min-h-screen pb-20 transition-colors ${
      ...
      <div className={`border-b sticky top-16 z-30 backdrop-blur-md transition-colors ${
        isDark ? 'border-stone-800 bg-stone-900/80' : 'border-slate-200 bg-white/90 shadow-xs'
      }`}>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 py-3">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto">
              <button
                ...
                className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-semibold transition-all whitespace-nowrap cursor-pointer ${
                  activeTab === 'overview'
                    ...
                    : isDark ? 'text-stone-300 hover:text-white hover:bg-stone-800' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
                }`}
              >
              ...
              >
                ...
                <span>Galerías & Sesiones ({galleries.length})</span>
              </button>
              ...
              >
                ...
                <span>Clientes & Permisos ({clientUsers.length})</span>
              </button>
              ...
              >
                ...
                <span className={`text-[10px] px-1.5 py-0.5 rounded font-mono-code font-bold ${
                  activeTab === 'storage' 
                    ...
                    : isDark ? 'bg-stone-950/80 text-amber-400' : 'bg-slate-200 text-slate-800'
                }`}>
              ...
              >
                ...
                <span>Personalización & Diseño</span>
              </button>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl font-bold text-xs transition-all shadow-md cursor-pointer text-white ${colorTheme.twBg} ${colorTheme.twBgHover} ${colorTheme.twShadow}`}
              >
              ...
              >
                <Users className={`w-4 h-4 ${colorTheme.twText}`} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-8">
              ...
              <div className={`lg:col-span-6 rounded-3xl border p-6 sm:p-8 space-y-6 ${
                ...
                <div className={`p-4 rounded-2xl border flex items-center justify-between ${
                  ...
                  <div className="text-xs">
                    ...
                    }`}>{formatBytes(storageStats.potentialSavingsBytes)}</p>
                  </div>
                  <button
                    id="overview-optimize-storage-btn"
                    onClick={() => setActiveTab('storage')}
        ...
        {/* TAB 4: SERVER STORAGE & FILE SIZE OPTIMIZER */}
        {activeTab === 'storage' && (
          <div className="space-y-8 animate-in fade-in duration-150">
            
            {/* Header with Server Quota & Storage Optimizer Tool */}
            <div className={`rounded-3xl border p-6 sm:p-8 space-y-6 shadow-xl ${
              ...
              <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b pb-6 ${
                ...
                <div>
                  <span className={`text-[10px] uppercase font-bold tracking-widest ${
                    ...
                    Optimización del Servidor Lumina
                  </span>
                  <h3 className={`text-2xl font-bold font-serif-display mt-1 ${
                    isDark ? 'text-stone-100' : 'text-slate-900'
                  }`}>
                    Control de Almacenamiento & Archivos de Alta Resolución
                  </h3>
                  <p className={`text-xs mt-1 max-w-xl ${isDark ? 'text-stone-400' : 'text-slate-500'}`}>
                    Supervisa el peso individual de cada archivo RAW/JPEG y genera vistas previas WebP optimizadas para reducir la carga del servidor sin comprometer la descarga en resolución original.
                  </p>
                </div>

                {/* Batch Optimize CTA */}
                <button
                  id="batch-optimize-server-btn"
                  onClick={onBatchOptimizeImages}
                  className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-gradient-to-r from-amber-500 to-emerald-500 hover:from-amber-400 hover:to-emerald-400 text-stone-950 font-bold text-xs transition-all shadow-lg shadow-emerald-500/20 cursor-pointer"
                >
                  <Sparkles className="w-4 h-4" />
                  <span>Optimizar Todo ({formatBytes(storageStats.potentialSavingsBytes)} de Ahorro)</span>
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className={`p-4 rounded-2xl border space-y-2 ${
                  ...
                  <p className="text-[11px] text-emerald-600 dark:text-emerald-400 font-medium">Cuota escalable NVMe SSD</p>
                </div>
                ...
                <div className={`p-4 rounded-2xl border space-y-2 ${
                  ...
                  <span className={`text-xs font-medium ${isDark ? 'text-stone-400' : 'text-slate-500'}`}>
                    Estado de Optimización WebP:
                  </span>
                  <p className="text-2xl font-bold font-mono-code text-emerald-600 dark:text-emerald-300">
                    {storageStats.optimizedImagesCount} / {storageStats.totalImagesCount} fotos
                  </p>
                  <p className={`text-[11px] ${isDark ? 'text-stone-400' : 'text-slate-500'}`}>
                    Generación de miniaturas inteligentes activada
                  </p>
                </div>
              </div>
            ...
            <form onSubmit={handleSaveGallery} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2 space-y-1">
                  ...
                  <div className={`grid grid-cols-1 sm:grid-cols-2 gap-2 p-3 rounded-xl border max-h-36 overflow-y-auto ${
                    isDark ? 'bg-stone-950 border-stone-800' : 'bg-slate-50 border-slate-200'
                  }`}>
                    {clientUsers.map((client) => {
                      const isChecked = gallerySelectedClients.includes(client.id);
                      return (
                        <label key={client.id} className={`flex items-center gap-2 text-xs cursor-pointer ${
