...
import { 
  GallerySession, GalleryImage, User, FeedbackItem, AuditLogItem, ServerStorageStats, StudioBrandingConfig 
} from './types';
import { 
...
export default function App() {
  ...
  const [currentUser, setCurrentUser] = useState<User | null>(() => {
    ...
    const usersList = loadUsersFromStorage();
  ...
  const serverStats: ServerStorageStats = useMemo(() => calculateServerStats(galleries, images), [galleries, images]);
      ...
      <footer className={`border-t py-6 text-xs transition-colors ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 text-[11px]">
            ...
            <span>Cuota: <strong className={`font-mono-code ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}>{serverStats.usedBytes ? (serverStats.usedBytes / (1024 * 1024)).toFixed(1) : 0} MB / 100 GB</strong></span>
